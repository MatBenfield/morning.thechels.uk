...
def get_covid_data(num_days):
    ...
    for i in range(0, 3):
        string_builder += f"<li>{0 if data[i]['newCases'] == None else data[i]['newCases']} new cases & {0 if data[i]['deaths'] == None else data[i]['deaths']} deaths "
        if i == 0:
...
if __name__ == "__main__":
    ...
    string_output = get_covid_data(3)
    final_output = replace_chunk(index_contents, "covid_marker", "<ul>" + string_output + "</ul>")
